...
> **Project status:** the portable foundation now includes checked rational timeline arithmetic, synchronized frame/audio intervals, validated CPU media buffers, capability and error types, asynchronous in-memory byte I/O, explicit CPU/GL/WebGL frame transfer contracts, normalized codec factories, and a bounded exact-frame decoder path with an uncompressed conformance backend. MP4, production compressed-codec integrations, playback, recording, and JavaScript APIs remain planned. The interfaces below describe the intended complete API and may change before the first release.
