...
`MediaOutput.finish()` returns a browser-owned `Blob` with the configured MIME type. `writeEncodedChunk()` remains the raw byte-sink boundary for a caller supplying its own already-muxed container bytes. Indexed `put` on any video track instead encodes through a built-in `WebCodecs` bridge (AV1 Main profile, RGBA8 input) and mixes every track's samples into a real, multi-track MP4 at `finish()`; call `videoEncodeSupport()` first to check whether the current browser can encode. Other codecs and audio `put` remain unsupported until their backends land.
