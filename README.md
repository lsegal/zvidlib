...
`MediaOutput.finish()` returns a browser-owned `Blob` with the configured MIME type. `writeEncodedChunk()` remains the raw byte-sink boundary for a caller supplying its own already-muxed container bytes. Indexed `put` on any video track encodes through a built-in `WebCodecs` bridge (AV1 Main or HEVC Main profile, RGBA8 input; `CreateOptions.videoCodec` selects which, defaulting to `"av1"`) and mixes every track's samples into a real multi-track MP4 at `finish()`; call `videoEncodeSupport()` first to check whether the current browser can encode the selected codec. Track 0 of an audio stream likewise encodes through a `WebCodecs` `AudioEncoder` bridge (AAC-LC, interleaved `f32` PCM input); call `audioEncodeSupport()` first. `finish()` muxes whichever tracks were used into one MP4. Other audio track indices and other codecs/profiles remain unsupported until their backends land.
