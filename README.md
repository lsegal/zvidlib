...
`MediaOutput.finish()` returns a browser-owned `Blob` with the configured MIME type. `writeEncodedChunk()` remains the raw byte-sink boundary for a caller supplying its own already-muxed container bytes. Indexed `put` on any video track encodes through a built-in `WebCodecs` bridge (AV1 Main or HEVC Main profile; `CreateOptions.videoCodec` selects which, defaulting to `"av1"`) and mixes every track's samples into a real multi-track MP4 at `finish()`; call `videoEncodeSupport()` first to check whether the current browser can encode the selected codec. Input frames accept `VideoFrame.rgba()`, `VideoFrame.bgra8()`, or `VideoFrame.yuv420p8()`. Track 0 of an audio stream likewise encodes through a `WebCodecs` `AudioEncoder` bridge (AAC-LC, interleaved `f32` PCM input); call `audioEncodeSupport()` first. `finish()` muxes whichever tracks were used into one MP4. Other audio track indices and other codecs/profiles remain unsupported until their backends land.
...
On macOS the same factory also reaches VideoToolbox's hardware HEVC encoder when `VideoEncoderConfig::hardware` is `Prefer` or `Require`, the host has one, and the configuration is a target bitrate: four big-endian bytes of bits a second, optionally followed by four big-endian bytes giving the maximum keyframe interval in frames (one second of frames when omitted). The software encoder accepts the eight-byte form too, treating it as the same bitrate, so a `Prefer` caller falls back without rewriting its configuration. The hardware encoder takes limited-range `Rgba8` or `Bgra8` CPU frames at any even size -- `Bgra8` is copied into the encoder's pixel buffers as it is and `Rgba8` has its channels swapped, and VideoToolbox converts either to YCbCr on the media engine, so no CPU colour conversion runs -- and emits HEVC Main in real-time mode with no B-frames, so every sample's DTS equals its PTS, with length-prefixed samples, sync flags from the encoder, and a standard `hvcC` built from the parameter sets it chose. `capability()` reports `CodecImplementation::Hardware` when that path is selected. It cannot honour the empty (lossless PCM) or one-byte (fixed QP) configurations, so those stay on the software encoder under `Prefer` and are an invalid configuration under `Require`, and every other target reports `CodecSupport::HardwareUnavailable` for `Require`. `finish()` drains the encoder; dropping it unfinished cancels, discarding the frames it had not yet emitted. On an Apple silicon host it encodes 1080p30 several times faster than real time.
...
Codec backends by platform. "Hardware" rows are what `HardwarePreference::Prefer` or `Require` reaches; `Avoid`, and `Prefer` where no hardware backend is available or the configuration is one it cannot honour, selects the software backend.

| Codec path | Windows | Linux | macOS | Browser |
| --- | --- | --- | --- | --- |
| HEVC Main decode | NVDEC or Media Foundation hardware, software fallback | NVDEC hardware, software fallback | VideoToolbox hardware, software fallback | `WebCodecs` |
| HEVC Main encode | Software | Software | VideoToolbox hardware (target bitrate, `Rgba8` or `Bgra8` input), software fallback | `WebCodecs` |
| AV1 Main decode | Software | Software | Software | `WebCodecs` |
| AV1 Main encode | Software (monochrome) | Software (monochrome) | Software (monochrome) | `WebCodecs` |
| AAC-LC encode | None | None | AudioToolbox | `WebCodecs` |
