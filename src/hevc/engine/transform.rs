...
mod tests {
    ...
    /// Small deterministic LCG so the benchmark's inputs are
    /// reproducible without `rand`, matching `transform_simd`'s tests.
    struct Lcg(u64);
    impl Lcg {
    ...
    /// Micro-benchmark for the §8.6 kernels: the §8.6.3 dequantization
    /// scaling loop and the §8.6.4.2 inverse-transform butterfly, timed
    /// separately, per transform size, on the scalar reference and on
    /// every SIMD backend the host CPU offers.
    ///
    ...
    ///   in the timing. This is what the backend itself is worth.
    /// * The `transform_1d` arm is split into a **dense** and a
    ///   **sparse** input. The kernels skip zero inputs outright, and a
    ///   real column of a dequantized block is mostly zero, so the two
    ///   can disagree: the sparse case does less vector work per call
    ///   while paying the same per-call overhead.
    /// * The whole §8.6 **block path** — [`scale_coefficients`] then
    ///   [`inverse_transform_with_backend`]. That path allocates and
    ///   zeroes a `Vec` per block and per intermediate, which is
    ///   identical work on every backend and compresses every ratio
    ///   towards 1.00x. It is here to show that compression, not to
    ///   stand in for the kernel figures.
    ///
    ...
    /// measured — this is the method #179 introduced for the §8.5.3.3
    ...
    /// simd_inverse_transform_benchmark -- --ignored --nocapture`.
    #[test]
    #[ignore = "benchmark; run with --ignored --nocapture"]
    fn simd_inverse_transform_benchmark() {
        use std::time::Instant;

        /// Timed passes per arm; the reported figure is the best of these.
        ...
        /// constant across sizes so the sizes are comparable to each
        /// other and not only to their own scalar baseline.
        const KERNEL_COEFFS: usize = 1 << 20;
        /// Blocks per size in the block-path arm.
        const PATH_BLOCKS: usize = 512;
        const BIT_DEPTH: u8 = 8;

        let backends = transform_simd::supported_backends();
        let mut rng = Lcg(0x8_6000);
        let mut sink = 0i64;

        // Per size: the sparse level block a real residual parse would
        // produce (energy in the low-frequency corner, the rest zero),
        // its dequantized dense counterpart, and the §8.6.3 parameters.
        struct SizeInputs {
            n_tbs: usize,
            levels: Vec<i32>,
            params: transform_simd::DequantParams,
            sparse_col: Vec<i32>,
            dense_row: Vec<i32>,
            row_step: usize,
        }
        let inputs: Vec<SizeInputs> = SIZES
            ...
            .map(|&n_tbs| {
                ...
                let log2_tbs = log2_tbs(n_tbs).expect("4/8/16/32");
                let q_p = 32u32;
                let mut levels = vec![0i32; count];
                let coded = (n_tbs / 2).max(4);
                for y in 0..coded {
                    for x in 0..coded {
                        levels[y * n_tbs + x] = rng.signed(32);
                    }
                }
                // A column of `d`: non-zero only where a coded
                // sub-block left something, i.e. the low `coded` rows.
                let sparse_col = (0..n_tbs)
                    .map(|y| if y < coded { rng.signed(1 << 12) } else { 0 })
                    .collect();
                // A row of the equation-8-314 `g` between the two
                // passes, which is dense even when `d` is not.
                let dense_row = (0..n_tbs).map(|_| rng.signed(1 << 12)).collect();
                SizeInputs {
                    n_tbs,
                    levels,
                    params: transform_simd::DequantParams {
                        level_scale: LEVEL_SCALE[(q_p % 6) as usize],
                        qp_div6: q_p / 6,
                        bd_shift: (i32::from(BIT_DEPTH) + log2_tbs as i32 + 10 - 15) as u32,
                        coeff_min,
                        coeff_max,
                    },
                    sparse_col,
                    dense_row,
                    row_step: 1usize << (5 - log2_tbs),
                }
            })
            .collect();

        // best[size][backend] = [dequant, transform_1d dense,
        // transform_1d sparse, block path].
        ...
        for _ in 0..BENCH_ROUNDS {
            for (s, input) in inputs.iter().enumerate() {
                let n = input.n_tbs;
                let count = n * n;
                let dequant_passes = KERNEL_COEFFS / count;
                let lane_passes = KERNEL_COEFFS / n;
                let basis = DCT32.as_flattened();
                for (b, &backend) in backends.iter().enumerate() {
                    let start = Instant::now();
                    for _ in 0..dequant_passes {
                        transform_simd::dequant_block(
                            backend,
                            &mut out[..count],
                            &input.levels,
                    ...
                    for _ in 0..PATH_BLOCKS {
                        let d = scale_coefficients(&input.levels, n, 32, BIT_DEPTH, false, None)
        ...
        println!(
            "\n§8.6 kernels, best of {BENCH_ROUNDS} interleaved rounds{}",
            if sink == i64::MIN { "!" } else { "" }
        );
        println!(
            "  isolated arms: {KERNEL_COEFFS} coefficients/pass, L1-resident, no allocation\n  \
             block path: {PATH_BLOCKS} blocks/pass through scale_coefficients + inverse_transform"
        );
        for (s, input) in inputs.iter().enumerate() {
            println!("  nTbS {}:", input.n_tbs);
            let baseline = best[s][0];
            for (b, &backend) in backends.iter().enumerate() {
                let [dequant, dense, sparse, path] = best[s][b];
                println!(
                    "    {:>8}  dequant_block {:7.2} ms ({:4.2}x)  transform_1d dense {:7.2} ms \
                     ({:4.2}x)  sparse {:7.2} ms ({:4.2}x)  block path {:7.2} ms ({:4.2}x)",
                    format!("{backend:?}"),
                    dequant * 1e3,
                    baseline[0] / dequant,
                    dense * 1e3,
