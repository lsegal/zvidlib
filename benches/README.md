...
several because its groups share the same decoded-frame cache; `audio_mux` and
`hevc_encode` share none of those fixtures and are separately runnable. The
encoder target is separate for a second reason too: its mode search is slow
enough that keeping it out of the default `cargo bench --bench codec` run is
worth more than sharing a process.

## Running

```sh
cargo bench                       # the default, fast groups in all targets
