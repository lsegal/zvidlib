...
`benches/hevc_hardware.rs` is a module of the same `codec` target (Cargo's
`autobenches` discovery is off so it is not mistaken for a second target). It
...
```sh
cargo bench --bench codec -- hevc_hardware                     # hardware arms only
ZVIDLIB_BENCH_LARGE=1 cargo bench --bench codec -- hevc_hardware  # plus the software baseline
```

Three things make this group different from the rest of the suite:
