...
// Each bench target (`benches/codec.rs`, `benches/audio_mux.rs`,
// `benches/hevc_encode.rs`) is its own crate root and compiles this whole
// module, but uses only the fixtures its own measurements need. Unused-here is
// not dead: `cargo clippy --all-targets` would otherwise fail one target for
